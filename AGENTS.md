...
Run these from the module root before checking off a task, syncing specs, or
archiving a change:
...
Before the first real package exists, these Python commands are not yet
meaningful. The first project-specific OpenSpec change should add the real
package layout and make the Definition of Done runnable from the module root.
