# opencode-executor Specification

## Purpose

Let the relay drive a real coding worker (OpenCode) that returns a patch and
tests, behind a mockable interface, with a file-scope guard that keeps a worker
from polluting the repo beyond its allowed paths.
