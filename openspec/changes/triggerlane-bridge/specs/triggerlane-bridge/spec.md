...
event and SHALL run only if an approver grants approval. If approval is denied, the node SHALL NOT run, SHALL be marked
`blocked`, and its dependents SHALL be `blocked`. The approval decision SHALL come
